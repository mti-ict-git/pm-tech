...
const BottomNav: React.FC = () => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-lg border-t border-slate-200 dark:border-slate-800 px-2 py-2 flex items-center justify-around pb-[env(safe-area-inset-bottom)] z-40">
      {navItems.map((item) => (
        <button
          key={item.label}
        ...
        >
          <span className={`material-symbols-outlined text-[24px] ${isActive(item.path) ? 'material-symbols-filled' : ''}`}>
            {item.icon}
          </span>
